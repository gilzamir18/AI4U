from ga3c import A3CMaster, A3CWorker
from gmproc import ClientServer
...
class AgentWrapper(RemoteEnv):
	...
	def step(self, action, value=None):
		...
		state['frame'] = np.array(self.buf, dtype=np.float32).reshape(1, 4, 20, 20)
	...
	def reset(self):
		state = self.step('reset')
		self.energy = state['energy']
		return state

envs = EnvironmentManager(4, wrapper=AgentWrapper)
