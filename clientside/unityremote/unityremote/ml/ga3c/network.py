...
class Network:
    ...
    def __init__(self, scope, n_actions, entropy_bonus, value_loss_coef, max_grad_norm, optimizer,
                 add_summaries, state_shape, make_inference_network, detailed_logs=False, debug=False, goal_inputs_shape=None):

        with tf.variable_scope(scope):
            #goal_inputs is a list of inputs describing the agent's goals.
            observations, action_logits, action_probs, value, layers = \
                make_inference_network(obs_shape=state_shape, n_actions=n_actions, debug=debug, goal_inputs_shape=goal_inputs_shape)
        ...
        if type(observations) is not tuple:
            raise Exception("First element of the output of the method make_inference_network" \
                            "\n must be a tuple of placeholders. Value found: {0}".format(observations))


        if len(observations) % 3 != 0:
            raise Exception("First element of the output of the method make_inference_network" \
                            "\n must be a tuple with size multiple of three. Value found: {0}".format(len(observations)))


        self.states = observations[0]
        self.goal_inputs = observations[1]
        self.goal = observations[2]
